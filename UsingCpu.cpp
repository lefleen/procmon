...
Result UsingCpuProc::get_time_using_cpu(const HANDLE& handle_process, ULARGE_INTEGER& time_using_cpu)
{
	...
	if (!GetProcessTimes(handle_process, &creation_time_process, &exit_time_process, &kernel_time_process, &user_time_process)) return Result::failure;
...
Result UsingCpuProc::calculate_total_using_cpu(unsigned long num_cores, HandleRAII& handle_process, double work_time)
{
	...
	if (get_time_using_cpu(handle_process.get(), all_time_using_cpu) == Result::failure || all_time_after_create.QuadPart == 0) return Result::failure;
...
Result UsingCpuProc::calculating_interval_using_cpu(unsigned long num_cores, long long pause_interval, HandleRAII& handle_process)
{
	double interval_using_cpu = 0;
	double interval_cpu_time = INTERVAL_CPU_TIME * 1.0E7;
	ULARGE_INTEGER time_work_process = { };

	if (get_time_using_cpu(handle_process.get(), time_work_process) == Result::failure || INTERVAL_CPU_TIME == 0 || OLD_CPU_TIME == 0)
	{
...
Result UsingCpuProc::update(HandleRAII& handle_process)
{
	...
	get_time_using_cpu(handle_process.get(), FULL_TIME_WORK_PROCESS);

	return Result::successful;
}

Result UsingCpuProc::calculate(long long pause_interval, HandleRAII& handle_process, double work_time_process)
{
	// Количество количества логических потоков
	unsigned long num_cores = std::thread::hardware_concurrency();

	// ОБщее использоание CPU
	if (calculate_total_using_cpu(num_cores, handle_process, work_time_process) == Result::failure)
		return Result::failure;

	// За определенный интервал времени
	if (calculating_interval_using_cpu(num_cores, pause_interval, handle_process) == Result::failure)
		return Result::failure;
